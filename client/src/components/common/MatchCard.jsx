...
const MatchCard = ({ match, onSendRequest }) => {
  ...
  const getTierInfo = (score) => {
    if (score >= 85) return { label: 'Excellent Match', bg: 'bg-emerald-50 text-emerald-700 border-emerald-200', ring: 'border-emerald-500' };
    if (score >= 70) return { label: 'Strong Match', bg: 'bg-indigo-50 text-indigo-700 border-indigo-200', ring: 'border-indigo-500' };
    if (score >= 50) return { label: 'Good Match', bg: 'bg-blue-50 text-blue-700 border-blue-200', ring: 'border-blue-500' };
    return { label: 'Potential Match', bg: 'bg-amber-50 text-amber-700 border-amber-200', ring: 'border-amber-500' };
  };

  const tier = getTierInfo(score);

  // Generate deterministic dynamic explanation
  const generateExplanation = () => {
    ...
    if (teachNames && learnNames) {
      return `You can teach ${teachNames}, which ${user.name} wants to learn, and ${user.name} can teach ${learnNames} to you.`;
    } else if (learnNames) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col justify-between hover:shadow-md transition-shadow relative overflow-hidden">
      <div>
        ...
        {/* Why this is a match explanation */}
        <div className="bg-indigo-50/60 rounded-xl p-3 mb-5 border border-indigo-100/70 text-xs text-indigo-900 flex items-start gap-2">
          <Sparkles className="w-4 h-4 text-indigo-600 shrink-0 mt-0.5" />
          <p className="leading-relaxed font-medium">
            <span className="font-bold">Why it's a match: </span>
            {generateExplanation()}
          </p>
        </div>

        {/* Complementary Skills Grid */}
        <div className="space-y-3.5 text-xs">
          <div>
            <h4 className="font-bold text-gray-700 mb-1.5 flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-indigo-600"></span> Skills You Can Teach:
            </h4>
            <div className="flex flex-wrap gap-1.5">
              {teachSkills.length > 0 ? (
                teachSkills.map((s) => (
                  <SkillBadge key={s._id} skill={s} size="sm" />
                ))
              ) : (
          ...
          <div>
            <h4 className="font-bold text-gray-700 mb-1.5 flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-600"></span> Skills You Can Learn:
            </h4>
            <div className="flex flex-wrap gap-1.5">
